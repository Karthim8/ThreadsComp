...
import { submitSonacseRegistration, verifySonacsePayment, loadEvents as loadEventsAPI } from '../services/api';
...
const CSERegister = () => {
    ...
    const validateStep1 = () => {
        ...
        // Validate based on year of study
        if (formData.year_of_study === '1') {
            if (!formData.admission_number?.trim()) newErrors.admission_number = 'SONACSE Admission Number is required';
            else if (formData.admission_number.length !== 10) newErrors.admission_number = 'Must be exactly 10 characters';
        } else {
            if (!formData.roll_number?.trim()) newErrors.roll_number = 'SONACSE Roll Number is required';
            else if (formData.roll_number.length !== 14) newErrors.roll_number = 'Must be exactly 14 characters';
        }
    ...
    const handleInputChange = (e) => {
        ...
        if (name === 'roll_number') {
            setFormData(prev => ({ ...prev, [name]: value.toUpperCase().slice(0, 14) }));
        } else if (name === 'admission_number') {
            setFormData(prev => ({ ...prev, [name]: value.toUpperCase().slice(0, 10) }));
        } else if (name === 'phone') {
            const numericValue = value.replace(/\D/g, '').slice(0, 10);
            setFormData(prev => ({ ...prev, [name]: numericValue }));
        } else if (name === 'year_of_study') {
            // Reset the roll_number/admission_number when year changes
            setFormData(prev => ({
                ...prev,
                [name]: value,
                roll_number: '', // Clear roll number
                admission_number: '' // Clear admission number
            }));
        } else {
    ...
    const calculateTotal = () => {
        ...
        if (formData.year_of_study === '1' && formData.event_selections.length > 0) {
            total += 250; // Flat rate for first year students no matter how many events
        }
    ...
    const handleSubmit = async () => {
        ...
        // Prepare payload based on year
        const payload = {
            ...formData,
            // For first year students, use admission_number as roll_number if needed by backend
            roll_number: formData.year_of_study === '1' ? formData.admission_number : formData.roll_number,
            accommodation_required: false // Default for SONACSE
        };

        // Remove admission_number from payload if backend doesn't expect it
    ...
    const handlePaymentRequired = async (data) => {
        ...
        const upiId = (data.payment_options && data.payment_options.upi_id) || 'rajikutty106@okaxis';
        ...
        const result = await Swal.fire({
            ...
            html: '<div class="text-left space-y-4 mb-4">' +
                ...
                '<p class="text-xs text-gray-400 break-all mt-1">' + upiId + '</p>' +
        ...
        if (result.isConfirmed) {
            ...
            const verifyPayload = {
                participant_id: participantId,            // top-level integer from backend
                transaction_id: txnId,
                amount: amount                            // must match DB pending total exactly
            };
            console.log('%c[SONACSE] Verify Payment Request:', 'color:#a78bfa;font-weight:bold', verifyPayload);
    ...
    const verifyPayment = async (paymentDetails) => {
        ...
        try {
            ...
            // error.message will contain the HTTP error from fetchAPI
            Swal.fire({
                icon: 'error',
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="col-span-1 md:col-span-2">
                                    <label className="block text-sm font-medium text-gray-400 mb-1">Year of Study <span className="text-red-500">*</span></label>
                                ...
                                {/* Conditional Input based on Year Selection */}
                                {formData.year_of_study && (
                                    <div className="col-span-1 md:col-span-2">
                                        {formData.year_of_study === '1' ? (
                                            <>
                                                <label className="block text-sm font-medium text-gray-400 mb-1">SONACSE Admission Number <span className="text-red-500">*</span></label>
                                                <div className="relative">
                                                    <input
                                                        type="text"
                                                        ...
                                                        className={`w-full bg-white/5 border ${errors.admission_number ? 'border-red-500' : 'border-white/10'} rounded-lg py-3 px-4 pl-12 text-white focus:outline-none focus:border-neon-cyan focus:ring-1 focus:ring-neon-cyan transition-all uppercase placeholder-gray-600`}
                                                    />
                                                ...
                                                <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
                                                    <Info size={12} className="text-neon-cyan" /> First Year students: Please enter your Admission Number.
                                                </p>
                                            </>
                                        ) : (
                                            <>
                                                <label className="block text-sm font-medium text-gray-400 mb-1">SONACSE REGISTER NO<span className="text-red-500">*</span></label>
                                                <div className="relative">
                                                    <input
                                                        type="text"
                                                        name="roll_number"
                                                        ...
                                                        className={`w-full bg-white/5 border ${errors.roll_number ? 'border-red-500' : 'border-white/10'} rounded-lg py-3 px-4 pl-12 text-white focus:outline-none focus:border-neon-cyan focus:ring-1 focus:ring-neon-cyan transition-all uppercase placeholder-gray-600`}
                                                    />
                                                    <Shield className="absolute left-4 top-3.5 text-gray-500 w-5 h-5" />
                                                </div>
                                                {errors.roll_number && <p className="text-red-500 text-xs mt-1">{errors.roll_number}</p>}
                                                <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
                                                    <Info size={12} className="text-neon-cyan" /> Name will be auto-filled by the system after validation.
                                                </p>
                                            </>
                                        )}
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            onChange={handleInputChange}
                                            className={`w-full bg-white/5 border ${errors.email ? 'border-red-500' : 'border-white/10'} rounded-lg py-3 px-4 pl-12 text-white focus:outline-none focus:border-neon-cyan focus:ring-1 focus:ring-neon-cyan transition-all`}
                                        />
                                ...
                                {/* Phone */}
                                <div>
                                    <label className="block text-sm font-medium text-gray-400 mb-1">Phone <span className="text-red-500">*</span></label>
                                    <div className="relative">
                                        <input
                                            type="tel"
                                            name="phone"
                            ...
                            <div className="bg-[#090011]/40 p-6 rounded-xl border border-white/10 space-y-4 font-mono text-sm shadow-inner">
                                <div className="flex justify-between border-b border-white/10 pb-2">
                                    ...
                                    <span className="text-white text-right">
                                        {formData.year_of_study === '1' ? 'First Year' :
                                            formData.year_of_study === '2' ? 'Second Year' :
                                                formData.year_of_study === '3' ? 'Third Year' : 'Fourth Year'}
                                    </span>
                                </div>
                                <div className="flex justify-between border-b border-white/10 pb-2">
                                    <span className="text-gray-400">
                                        {formData.year_of_study === '1' ? 'Admission Number' : 'Roll Number'}
                                    </span>
                                    <span className="text-white text-right">
                                        {formData.year_of_study === '1' ? formData.admission_number : formData.roll_number}
                                    </span>
                                </div>
                                <div className="flex justify-between border-b border-white/10 pb-2">
